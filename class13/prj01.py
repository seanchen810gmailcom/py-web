...
import openai # pip install openai；這個套件負責幫我們和 OpenAI 的 API 溝通，讓我們可以用 ChatGPT 的功能
#######################初始化#######################
...
openai.api_key = os.getenv("OPENAI_API_KEY") # 從環境變數拿到OpenAI API金鑰，準備給 openai 這個套件使用，讓我們可以用 ChatGPT 的功能
...
async def weather(interaction: discord.Interaction, city_name: str,forecast:bool = False,ai: bool = False):
    ...
    # OpenAI呼叫失敗的原因和天氣API不同（例如額度不足、網路過時）
    # 所以用建立的try/except區塊來分開處理，讓錯誤訊息更清楚。
    try:
        response = openai.chat.completions.create(
            model="gpt-4o",
            messages=[
                {"role": "system", "content": "你是一個專業的氣象分析師，能夠從天氣預報的原始資料中，分析出對使用者有用的天氣摘要。"},
                {"role": "user", "content": f"以下是{city}的未來天氣預報，請根據這些數據提供詳細的天氣分析建議：{raw_forecast}"},
            ],
            temperature=0.2, # 這個參數控制回覆的創意程度，0.7 是一個適中的值，讓回覆
        )
        annalysis = response.choices[0].message.content # 從OpenAI的回覆裡拿到分析結果
        await interaction.followup.send(f"**{city}**的天氣預報分析：\n{annalysis}") # 把分析結果回覆給使用者
    except openai.OpenAIError  as e:
        await interaction.followup.send(f"分析天氣預報時發生錯誤：{str(e)}")
