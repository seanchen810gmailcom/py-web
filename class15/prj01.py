...
每次回覆請控制在500個中文字以內，避免Discord 訊息太長。
...
同時，你也是一個非常非常兇的人，誰跟你講話，你都要好好罵他一頓，把它嗆到爆炸
"""
...
async def ask_with_discord_history(message):
    """當機器人笨提到時，只裡Discord 歷史，再交給AI助手回答。"""
    ...
    # Discord 提到機器人時，訊息裡會出現<@機器人ID>
    # 這個標記是給Discord 辨認用的，交給AI前先拿掉，問題會比計較乾淨
    user_question = message.content.replace(f"<@{bot.user.id}>", "").strip()
    if not user_question:
        user_question = "請根據前面的頻道對話，接著回影大家。"

    # 把提問者名字也放進內容中，AI回覆多人聊天時會比較清楚
    user_message = (
        f"{message.author.display_name}"
        f"(mention：{message.author.mention})提示你：{user_question}"
    )

    # 上一堂的ask多了history_messages參數；
    # 沒有歷史時可以不傳，有歷史時就把整理好的舊對話一起交給AI。
...
async def on_message(message):
    ...
    if message.content == "hello":  # 如果這則訊息的內容是 Hello
        ...
        print(
            "有人打招呼了！"
        )  # 在終端機印出「有人打招呼了！」，讓我們知道這件事發生了

    #!會傳到頻道裡的每個人
    elif bot.user in message.mentions:
        # 只有在群組頻道中被@時，才讀取前面的頻道訊息當上下文
        async with message.channel.typing():
            answer, error = await ask_with_discord_history(message)
        if error:
            await message.channel.send(error)
        else:
            # 用reply可以清楚接再發問者的訊息下面；
            # allowed_mentions則控至AI回覆裡哪些mention真的會生效
            await message.reply(
                answer, mention_author=True, allowed_mentions=AI_RETRY_ALLOWED_MENTTIONS
            )
...
async def weather(
    ...
    analysis, error = ai_assistant.ask(
        system_prompt="你是一個專業的氣象分析師，能夠從天氣預報的原始資料中，分析出對使用者有用的天氣摘要。請控制在500字以內",
