...
def build_forecast_embeds(forecast_summary):
    """把未來多筆預報資料排程Discord卡片清單。"""
    # forecast_summary 裡每一筆是同一座城市、不同時間點的天氣資料。
    ...
    for forecast in forecast_summary:
        ...
        # forecast 的 icon_code也是WeatherAPI 整理好的資料，可以直接拿來組圖示網址
        icon_url=weather_api.get.get_icon_url(forecast["icon_code"])
        embed.set_thumbnail(url=icon_url)
        embed.add_field(
            name="溫度",
...
async def weather(interaction: discord.Interaction, city_name: str,forecast:bool = False):
